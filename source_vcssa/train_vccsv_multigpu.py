...
from distributed_utils import get_rank, is_main_process, reduce_value, get_world_size
...
def train(net, train_loader, eval_loader, optim, args, device, scheduler):
    if is_main_process():
        logfile = open(
    ...
    emotion_label_map = read_json_file(
        os.path.join(args.datadir, args.emotion_label_map))  # label mapping file of emotion
    opinion_label_map = read_json_file(
        os.path.join(args.datadir, args.opinion_label_map))  # label mapping file of opinion
    ...
    for epoch in range(0, args.max_epoch):  # epoch loop
        ...
        for step, batch_data in enumerate(train_loader):  # step : a "backward"
            ...
            epoch_loss = epoch_op_loss + epoch_em_loss 
            ...
            if args.aux_task == True:
                print("\r[Epoch %2d][Step %4d/%4d] Loss_sum: %.4f,opinion_loss: %.4f, emotion_loss: %.4f, \
                        opinion_aux_loss: %.4f, emotion_aux_loss: %.4f, Lr: %.2e, %4d m "
                      "remaining" % (
                          epoch + 1,
                          step,
                          int(len(train_loader.dataset) / (args.batch_size * get_world_size())),
                          loss,
        ...
        if is_main_process():
            ...
            if args.aux_task == True:
                ...
                logfile.write(
                    ...
                    ', Loss: ' + str(epoch_loss / len(train_loader.dataset)) +
                    ', opinion_loss' + str(epoch_op_loss) +
                    ', emotion_loss' + str(epoch_em_loss) +
                    ', opinion_aux_loss' + str(op_aux_loss_sum) +
                    ', emotion_aux_loss' + str(emo_aux_loss_sum) +
        ...
        if epoch_finish >= args.eval_start:
            ...
            if is_main_process():
                write_pkl_file(args.output + "/" + args.name + '/' + "dev_predict_" + str(epoch_finish) + ".pkl",
                               result)
...
@torch.no_grad()
def evaluate(net, eval_loader, args, opinion_label_num=3, opinion_label_map=None, emotion_label_num=8,
    ...
    with torch.no_grad():
        for step, batch_data in enumerate(eval_loader):
            ...
            # predict
            output = net(
                batch_data)  # {'opinion_predict':tensor([[0.3335, 0.2738, 0.3926],[0.3429, 0.2711, 0.3860],[0.3410, 0.2730, 0.3860], [0.3411, 0.2715, 0.3874]], device='cuda:0', grad_fn=<SoftmaxBackward0>)'emotion_predict':tensor([[0.0940, 0.1774, 0.0932, 0.1248, 0.1377, 0.1427, 0.1352, 0.0952],}
    ...
    accuracy['opinion'] = {
        "macro": \
            {"precision": opinion_precision_macro, "recal": opinion_recall_macro, "f1_score": opinion_f1_macro}, \
        "micro": \
            {"precision": opinion_precision_micro, "recal": opinion_recall_micro, "f1_score": opinion_f1_micro}, \
        "class": \
            {"precision": opinion_precision_class.tolist(), "recal": opinion_recall_class.tolist(),
    ...
    accuracy['emotion'] = {
        "macro": \
            {"precision": emotion_precision_macro, "recal": emotion_recall_macro, "f1_score": emotion_f1_macro}, \
        "micro": \
            {"precision": emotion_precision_micro, "recal": emotion_recall_micro, "f1_score": emotion_f1_micro}, \
        "class": \
            {"precision": emotion_precision_class.tolist(), "recal": emotion_recall_class.tolist(),
